...
int omap2_clk_enable(struct clk *clk)
{
	...
	if (clk->usecount > 1){
		...
		/*Because we have changed these two clk's flag into ENABLE_ON_INIT,it will call clk_enable(clkp),
		usecount will increase. when suspend these two clk won't really be disable. this is the reason we need to decrease the usecount*/
		if(strcmp(clk->name, "dss_sys_clk") == 0 && clk->usecount == 3)
			clk->usecount = 2;
		if(strcmp(clk->name, "dss_dss_clk") == 0 && clk->usecount == 4)
			clk->usecount = 3;
#endif
		return 0;
	}
...
void omap2_clk_disable_unused(struct clk *clk)
{
	...
	if (clk->clkdm != NULL) {
		pwrdm_wait_transition(clk->clkdm->pwrdm.ptr);
		pwrdm_state_high2low_counter_update(clk->clkdm->pwrdm.ptr);
	}
}
#endif

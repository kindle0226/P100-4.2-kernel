...
// Extra flag for C1 device using
#define EXPANSION_IO_CTRL
...
/* regulators to camera 1.2V power supply */
static struct regulator *vdd_1v2 = NULL;
//static struct regulator *cam_1v2 = NULL;
...
#ifdef EXPANSION_IO_CTRL
#define GPIO_CAM_EN1V2 			277
#define GPIO_CAM_AF                     282
#define GPIO_CAM_XSHUTDOWN              283

static int camera_power_on(){
	if (!vdd_1v2) {
		vdd_1v2 = regulator_get(NULL, "cam_v1v2");
		if (IS_ERR_OR_NULL(vdd_1v2)) {
			vdd_1v2 = NULL;
			printk(KERN_ERR "%s: Failed to initialize vdd 1.2v regulator..\n", __func__);
			return -ENOENT;
		}
	}

	//TODO: use gpio regulator to cotrol CAM_EN1V2



	regulator_enable(vdd_1v2);
	gpio_direction_output(GPIO_CAM_EN1V2, 0);
	msleep(20);

	gpio_direction_output(GPIO_CAM_AF, 1);
	msleep(20);

	gpio_direction_output(GPIO_CAM_XSHUTDOWN, 1);
	msleep(20);

	return 0;
}

static int camera_power_off(){
	if (!vdd_1v2) {
		printk(KERN_ERR "%s: Failed to get 1.2v regulators\n", __func__);
		return -ENOENT;
	}

	regulator_disable(vdd_1v2);
	gpio_direction_output(GPIO_CAM_EN1V2, 1);
	gpio_direction_output(GPIO_CAM_AF, 0);
	gpio_direction_output(GPIO_CAM_XSHUTDOWN, 0);
...
#ifdef EXPANSION_IO_CTRL
	if (camera_power_on()) return -ENOENT;
#endif
...
#ifdef EXPANSION_IO_CTRL
	gpio_request(GPIO_CAM_EN1V2, "GPIO_CAM_EN1V2");
	gpio_request(GPIO_CAM_AF, "GPIO_CAM_AF");
	gpio_request(GPIO_CAM_XSHUTDOWN , "GPIO_CAM_XSHUTDOWN ");
#endif
...
#ifdef EXPANSION_IO_CTRL
	gpio_free(GPIO_CAM_EN1V2);
	gpio_free(GPIO_CAM_AF);
	gpio_free(GPIO_CAM_XSHUTDOWN);
#endif
}

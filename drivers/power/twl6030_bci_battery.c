...
// temp info for bq2415x using
volatile int bat_temp = 0;

//TODO:we should check hw version to turn on/off these configs
//#define DEV_FORCE_OCV
#define DEV_NO_BACKUP_BAT
#define DEV_FORCE_VAC

#ifdef DEV_FORCE_OCV
extern bool board_is_c1_evt1(void);
#include <linux/time.h>
#define MIN_OCV_PERIOD 10/*minimal interval of OCV in second*/
#define CAP_DROP_THRESHOLD 3/*threshold to detect rapid drop of capacity*/
//NOTE: The following table is measured at 25°C with 315mA discharging current
// update at 20141226, use reported power level instead of actual power level
// RPL can be converted from APL by using "RPL = (APL - 10)/(100 - 10)"
...
static void twl6030_start_ac_charger(struct twl6030_bci_device_info *di)
{
	...
	/*we should not enable charger when OTG is in HOST mode*/
	u8 hw_state;
...
static void twl6032_charger_ctrl_interrupt(struct twl6030_bci_device_info *di)
{
	...
	err = twl_i2c_read_u8(TWL6030_MODULE_CHARGER, &present_status,
			CHARGERUSB_INT_STATUS);
	if (err < 0) {
		dev_err(di->dev, "%s: Error access to TWL6030 (%d)\n",
			__func__, err);
...
static int capacity_changed(struct twl6030_bci_device_info *di)
{
	...
	/* Check temperature for max charging voltage */

	/* Update temperature for bq2415x */
	bat_temp = di->temp_C;
	...
	{
		// NOTE: This is a SW patch for the capacity display in recharge stage.
		//       It will store the nac and recalculate a new capacity based on
		//       the stored nac, so user would always see 100% in recharge stage.
		static int nac_diff = 0;
		if (di->charge_status != POWER_SUPPLY_STATUS_CHARGING) {
			// discharging
			if (di->cell.soc < MAX_PERCENTAGE && di->cell.cc) {
				nac_diff = di->cell.fcc - di->cell.nac;
				di->cell.soc = MAX_PERCENTAGE;
			}
		} else {
			// charging
			if (di->cell.nac + nac_diff >= di->cell.fcc) {
				nac_diff = di->cell.fcc - di->cell.nac;
				di->cell.soc = MAX_PERCENTAGE;
			}
		}

		if (nac_diff && !di->cell.edv2)
			di->cell.soc = DIV_ROUND_CLOSEST((di->cell.nac + nac_diff)*
				MAX_PERCENTAGE, di->cell.fcc);
	}
...
#ifdef DEV_FORCE_OCV
static int voltage_changed(struct twl6030_bci_device_info *di)
{
	static int last_cell_soc = 0;
	...
	do_gettimeofday(&tv);
	if (last_cell_soc && tv.tv_sec - last_tv.tv_sec <= MIN_OCV_PERIOD) return 0;

	twl_i2c_read_u8(TWL6030_MODULE_CHARGER, &reg, CONTROLLER_CTRL1);
	if (reg) {
	...
	if (reg) twl_i2c_write_u8(TWL6030_MODULE_CHARGER, reg, CONTROLLER_CTRL1);
	
	for(i = 0; i < sizeof(battery_level)-1;i++)
	{
             if(di->voltage_mV < battery_level[i+1])
                 break;
        }
	bat_level = i;
        bat_percentage = (bat_level == sizeof(battery_level)-1)? 100 :
           (di->voltage_mV - battery_level[i]) * 5 /(battery_level[i+1] - battery_level[i]) + 5 * i;

	if (bat_percentage < 0) bat_percentage = 0;


	di->cell.soc = bat_percentage;

	//printk("batery volage = %drate = %d \n", di->voltage_mV, val->intval);
...
#ifndef DEV_NO_BACKUP_BAT
	ret = power_supply_register(&pdev->dev, &di->bk_bat);
	if (ret) {
		dev_dbg(&pdev->dev, "failed to register backup battery\n");
		goto bk_batt_failed;
	}
#endif /*DEV_NO_BACKUP_BAT*/
	di->charge_n1 = 0;
	di->timer_n1 = 0;

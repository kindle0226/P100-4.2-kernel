...
extern volatile int bat_temp;
extern bool board_is_c1_dvt1_atcn(void);
extern bool board_is_c1_dvt1_atce(void);
...
static void bq2415x_charger_work(struct work_struct *work)
{
	...
	/* check temperature then change charging voltage if needed */
	if (board_is_c1_dvt1_atcn() || board_is_c1_dvt1_atce()) {
		static bool high_temp = false;
		static int orig_voltagemV = 0;

		// check if voltagemV has been updated in other processes
		if (orig_voltagemV - ((high_temp)?200:0) > di->voltagemV)
			orig_voltagemV = di->voltagemV;

		// check if battery temperature is higher than 49.5 degree
		if (bat_temp > 495 && !high_temp) {
			orig_voltagemV = di->voltagemV;
			// reduce the charging voltage by 200 mV
			di->voltagemV -= 200;
			high_temp = true;
			bq2415x_config_voltage_reg(di);
		} else if (high_temp && bat_temp <= 495) {
			// restore the charging voltage
			di->voltagemV = orig_voltagemV;
			high_temp = false;
			bq2415x_config_voltage_reg(di);
		}
	}

...
#define DISPC_ISR_BIT_ONESHOT  0

#define DISPC_ISR_FLAG_ONESHOT (1 << DISPC_ISR_BIT_ONESHOT)
...
static int omap_dispc_register_wait_isr(u32 mask, struct completion **compl);
static int omap_dispc_unregister_wait_isr(u32 mask, struct completion **compl);
...
static void dispc_mgr_enable_lcd_out(enum omap_channel channel, bool enable)
{
	...
	if (!enable && is_on) {
        r = omap_dispc_register_wait_isr(irq, &frame_done_completion);
		if (r)
	...
	if (!enable && is_on) {
		...
		r = omap_dispc_unregister_wait_isr(irq, &frame_done_completion);
		if (r)
			printk("WARN: failed to unregister FRAMEDONE isr. Maybe it is auto-unregistered\n");
	}
}
...
static void dispc_mgr_enable_digit_out(bool enable)
{
    ...
    r = omap_dispc_register_wait_isr(irq_mask, &frame_done_completion);
	if (r)
	...
	r = omap_dispc_unregister_wait_isr(irq_mask, &frame_done_completion);
	if (r)
 ...
 * @return 
 */
static int _omap_dispc_unregister_isr_nosync(omap_dispc_isr_t isr, void *arg, u32 mask)
{
    int i;
    int ret = -EINVAL;
    struct omap_dispc_isr_data *isr_data;
    ...
    for (i = 0; i < DISPC_MAX_NR_ISRS; i++) {
        isr_data = &dispc.registered_isr[i];
        if ((isr_data->isr != isr &&  isr_data->isr != NULL) || isr_data->arg != arg ||
                isr_data->mask != mask)
            continue;
        if (isr_data->isr) {
            set_irq = 1;
...
/**
 * @brief Special handler for one-shot irq wait
 *
 * @param data
 * @param mask
 */
static void dispc_irq_wait_handler(void *data, u32 mask)
{
	...
	_omap_dispc_unregister_isr_nosync(dispc_irq_wait_handler, data, mask);
	spin_unlock_irqrestore(&dispc.irq_lock, flags);
#endif
    WARN(1, "%s(): Should never run here! \n", __func__);
}

static int _omap_dispc_register_isr(omap_dispc_isr_t isr, void *arg, u32 mask, u32 flag, struct completion **compl)
{
	int i;
	int ret;
	struct omap_dispc_isr_data *isr_data;
	...
	for (i = 0; i < DISPC_MAX_NR_ISRS; i++) {
		...
		if (isr_data->isr != NULL || isr_data->flag)
			continue;

		isr_data->isr = isr;
		isr_data->mask = mask;
		isr_data->flag = flag;
		INIT_COMPLETION(isr_data->compl);
		if (flag & DISPC_ISR_FLAG_ONESHOT) {
			isr_data->arg = &isr_data->compl;
		} else {
			isr_data->arg = arg;
		}
		ret = 0;
...
static int omap_dispc_register_wait_isr(u32 mask, struct completion **compl)
{
	...
	spin_lock_irqsave(&dispc.irq_lock, flags);
	WARN((ret = _omap_dispc_register_isr(dispc_irq_wait_handler, NULL, mask, DISPC_ISR_FLAG_ONESHOT, compl)),
				"OMAP DISPC could not register wait irq ret=%d \n", ret);
	spin_unlock_irqrestore(&dispc.irq_lock, flags);
	return ret;
...
int omap_dispc_register_isr(omap_dispc_isr_t isr, void *arg, u32 mask)
{
	...
	spin_lock_irqsave(&dispc.irq_lock, flags);
	ret = _omap_dispc_register_isr(isr, arg, mask, 0x0, NULL); 
	spin_unlock_irqrestore(&dispc.irq_lock, flags);
	return ret;
}
...
int omap_dispc_unregister_isr_nosync(omap_dispc_isr_t isr, void *arg, u32 mask)
{
	...
	spin_lock_irqsave(&dispc.irq_lock, flags);
	ret = _omap_dispc_unregister_isr_nosync(isr, arg, mask);
	spin_unlock_irqrestore(&dispc.irq_lock, flags);
	return ret;
}
...
static int omap_dispc_unregister_wait_isr(u32 mask, struct completion **compl)
{
	...
	arg = *compl;
	*compl = NULL;
	return omap_dispc_unregister_isr_sync(dispc_irq_wait_handler, arg, mask);
}
...
static irqreturn_t omap_dispc_irq_handler(int irq, void *arg)
{
	...
	struct omap_dispc_isr_data *isr_data;
	unsigned long flags;
    int old_irq_status = g_already_irq;
    irqreturn_t ret = IRQ_HANDLED;
    int set_irq = 0;

	spin_lock_irqsave(&dispc.irq_lock, flags);
    if (old_irq_status) {
        printk("%s(): we got recursive interrupt. \n", __func__);
    }
    g_already_irq = 1;
...
#ifdef CONFIG_OMAP2_DSS_COLLECT_IRQ_STATS
	{
		...
		//BUT: CONFIG_OMAP2_DSS_COLLECT_IRQ_STATS is not enabled ?
		unsigned long stats_flags;
		spin_lock_irqsave(&dispc.irq_stats_lock, stats_flags);
		dispc.irq_stats.irq_count++;
		dss_collect_irq_stats(irqstatus, dispc.irq_stats.irqs);
		spin_unlock_irqrestore(&dispc.irq_stats_lock, stats_flags);
	}
#endif
	 ...
	 * themselves. */
	/* FIXME: Is this good idea? It does not help to avoid race-condition ?
	 For example: 
	 core0		 core1
	 copy reg_isr  running isr ?
	 copy reg_isr  stop isr -> unregister
	 call isr----> BOOM 
	 */
#if 0
	...
	{
		isr_data = &dispc.registered_isr[i];
		if (likely(isr_data->isr)) {
			if (unlikely(isr_data->mask & irqstatus)) {
				//If this is one-shot wake up, let do it here
				if (unlikely(test_bit(DISPC_ISR_BIT_ONESHOT, &isr_data->flag))) {
					complete_all(&isr_data->compl);
					//Auto unregister
					isr_data->isr = NULL;
					//isr_data->flag = 0;
                    set_irq = 1;
				} else {
					//Unlock here to call periodic ISR or we maybe have dead-lock
					spin_unlock_irqrestore(&dispc.irq_lock, flags);
...
static int omap_dispc_wait_for_irq(u32 irqmask, unsigned long timeout, int interruptable)
{
	int r;
	struct completion *compl = NULL;

	r = omap_dispc_register_wait_isr(irqmask, &compl); 
	if (r || !compl)
		return r;

	if (interruptable) {
		timeout = wait_for_completion_interruptible_timeout(compl, timeout);
	} else {
		timeout = wait_for_completion_timeout(compl, timeout);
	}
	//FIXME: if there is timeout, the handler will wake-up nothing here
	//and the dispc_irq_wait_handler will unregister it automatically
	//In case timeout, we need to call this or we out of ISR slot
	omap_dispc_unregister_wait_isr(irqmask, &compl);
...
{
	return omap_dispc_wait_for_irq(irqmask, timeout, 0);
}
...
{
	return omap_dispc_wait_for_irq(irqmask, timeout, 1);
}
...
static int omap_dispchw_probe(struct platform_device *pdev)
{
	...
	printk("OMAP DISPC: Init completion struct \n");
	for (i = 0; i < DISPC_MAX_NR_ISRS; i++) {
		init_completion(&dispc.registered_isr[i].compl);
	}
	...
	dev_info(&pdev->dev, "OMAP DISPC rev %d.%d\n",
		   FLD_GET(rev, 7, 4), FLD_GET(rev, 3, 0));
	dev_info(&pdev->dev, "OMAP DISPC: Enhance oneshot ISR .... \n");

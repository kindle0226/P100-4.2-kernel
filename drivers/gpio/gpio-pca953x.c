...
{
	struct pca953x_chip *chip = irq_data_get_irq_chip_data(d);
	uint32_t level = d->irq - chip->irq_base;
...
int pca953x_irq_set_wake(struct irq_data *data, unsigned int on)
{
	// we should do this in drivers/input/keyboard/gpio_keys.c rather than here
#if 0
	struct pca953x_chip *chip = irq_data_get_irq_chip_data(data);
	if(data->irq == gpio_to_irq(CAM_SHOT_GPIO)){
		if(on){
			is_from_resume = true;
			return enable_irq_wake(gpio_to_irq(chip->client->irq));
		}else{
			is_from_resume = false;
			return disable_irq_wake(gpio_to_irq(chip->client->irq));
		}
	}
	return -ENXIO;
#endif
...
static uint32_t pca953x_irq_pending(struct pca953x_chip *chip)
{
	u32 cur_stat;
	uint32_t old_stat;
	uint32_t pending;

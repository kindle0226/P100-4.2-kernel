...
#include <linux/twl6030-pwrbutton.h>
#include "bstclass.h"
...
/*! Bosch sensor remapping table size P0~P8, P8 is for C1 only*/
...
static struct regulator *bma2x2_2v8;
static struct regulator *bma2x2_1v8;

#if defined(CONFIG_ORIENT_MOTION) || defined(CONFIG_DOUBLE_TAP)
static bool irq_enabled_in_suspend = false;
#endif
...
struct bma2x2_data {
	...
	atomic_t delay;
	atomic_t enable;
#ifdef CONFIG_ORIENT_MOTION
	atomic_t ornt_enable;
#endif
...
static int bma2x2_set_orient_ud_en(struct i2c_client *client, unsigned char
		thetablk)
{
	int comres = 0;
	unsigned char data;

	comres = bma2x2_smbus_read_byte(client, BMA2X2_ORIENT_UD_EN__REG, &data);
	data = BMA2X2_SET_BITSLICE(data, BMA2X2_ORIENT_UD_EN, thetablk);
	comres = bma2x2_smbus_write_byte(client, BMA2X2_ORIENT_UD_EN__REG,
			&data);
...
static int bma2x2_get_orient_ud_en(struct i2c_client *client, unsigned char
		*status)
{
	int comres = 0;
	unsigned char data;
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_ORIENT_UD_EN__REG, &data);
	data = BMA2X2_GET_BITSLICE(data, BMA2X2_ORIENT_UD_EN);
	*status = data;
...
static ssize_t bma2x2_orient_uden_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (bma2x2_get_orient_ud_en(bma2x2->bma2x2_client, &data) < 0)
		return sprintf(buf, "Read error\n");
...
static ssize_t bma2x2_orient_uden_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (bma2x2_set_orient_ud_en(bma2x2->bma2x2_client, (unsigned
					char)data) < 0)
		return -EINVAL;
...
#if 1
	if(result)
		printk(KERN_INFO "Fail:BMX055 9-axis sensor (Accelerometer) self test.\n");
	else
		printk(KERN_INFO "Pass:BMX055 9-axis sensor (Accelerometer) self test.\n");
#endif
	return count;
}
...
#ifdef CONFIG_ORIENT_MOTION
static ssize_t bma2x2_orient_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", atomic_read(&bma2x2->ornt_enable));

}

static int bma2x2_set_en_orient_motion(struct bma2x2_data *bma2x2,
		int en)
{
	int err = 0;
	...
	if (atomic_read(&bma2x2->ornt_enable) != en) {
		if (en)
		{
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_NORMAL);
			err = bma2x2_set_Int_Enable(bma2x2->bma2x2_client, 10, 1);
		#ifdef BMA2X2_ENABLE_INT1
			err += bma2x2_set_int1_pad_sel(bma2x2->bma2x2_client, PAD_ORIENT);
		#else
			err += bma2x2_set_int2_pad_sel(bma2x2->bma2x2_client, PAD_ORIENT);
		#endif
		}
		else
		{
			err = bma2x2_set_Int_Enable(bma2x2->bma2x2_client, 10, 0);
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND);
		}
		atomic_set(&bma2x2->ornt_enable, en);
	}
	return err;
}

static ssize_t bma2x2_orient_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (error)
		return error;
	if ((data == 0) || (data == 1))
		bma2x2_set_en_orient_motion(bma2x2, data);
...
static DEVICE_ATTR(orient_enable, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bma2x2_orient_enable_show, bma2x2_orient_enable_store);
...
static DEVICE_ATTR(orient_uden, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bma2x2_orient_uden_show, bma2x2_orient_uden_store);
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	case 0x40:
		...
		printk(KERN_INFO "orient interrupt happened,%s\n",
				orient[first_value]);
		if(first_value == 1 || first_value == 5)
		{
			if(suspend_flag && atomic_read(&bma2x2->ornt_enable) == 1)
			{
				input_report_key(pbutton.input_dev, pbutton.report_key, 1);
				input_sync(pbutton.input_dev);
		...
		if (first_value == 0)
			...
			input_report_abs(bma2x2->dev_interrupt,
				...
				printk("orient interrupt happened\n");
		...
		const struct i2c_device_id *id)
{
	...
	bma2x2_2v8= regulator_get(NULL, "bmx_v2v8");
	if (IS_ERR_OR_NULL(bma2x2_2v8)) {
		printk("Failed to initialize msg21xxa regulator..\n");
		return -ENODEV;
	}
	err = regulator_enable(bma2x2_2v8);
	if (err) {
		printk("Failed to enabling msg21xxa regulator\n");
		regulator_put(bma2x2_2v8);
		return err;
	}

	bma2x2_1v8= regulator_get(NULL, "bmx_v1v8");
	if (IS_ERR_OR_NULL(bma2x2_1v8)) {
		printk("Failed to initialize msg21xxa regulator..\n");
		return -ENODEV;
	}
	err = regulator_enable(bma2x2_1v8);
	if (err) {
		printk("Failed to enabling msg21xxa regulator\n");
		regulator_put(bma2x2_1v8);
		return err;
	}
#endif
	...
	/*qmqiu orient interrupt enable*/
	//bma2x2_set_Int_Enable(client, 10, 1); 
	/*qmqiu orient_uden disable*/
	bma2x2_set_orient_ud_en(client,0);  
#ifdef CONFIG_BMA_ENABLE_NEWDATA_INT
	...
	err = request_irq(data->IRQ, bma2x2_irq_handler, IRQF_TRIGGER_RISING,
			"bma2x2", data);
	#else
	if (gpio_request(client->irq, "bma2x2") < 0)  {
		dev_err(&client->dev, "GPIO request failed\n");
	}
	gpio_direction_input(client->irq);
	data->IRQ = gpio_to_irq(client->irq);
	err = request_threaded_irq(data->IRQ, NULL, bma2x2_irq_handler, IRQF_TRIGGER_RISING,
				"bma2x2", data);
	#endif
...
#endif
#ifdef CONFIG_ORIENT_MOTION
	atomic_set(&data->ornt_enable, 0);
	// Disable orientation interrupt
	bma2x2_set_en_orient_motion(data, 0);
	// Enable orientation interrupt
	//bma2x2_set_en_orient_motion(data, 1);
#endif
#ifdef CONFIG_DOUBLE_TAP
	atomic_set(&data->en_double_tap, 0);
	// Disable double tap interrupt
	bma2x2_set_en_double_tap(data, 0);
	// Enable double tap interrupt
	//bma2x2_set_en_double_tap(data, 1);
	// Set tap threshold to 4
	bma2x2_set_tap_threshold(data->bma2x2_client, 4);
	data->tap_times = 0;
...
#if defined(CONFIG_ORIENT_MOTION) || defined(CONFIG_DOUBLE_TAP)
#if defined(CONFIG_ORIENT_MOTION) && defined(CONFIG_DOUBLE_TAP)
	if(atomic_read(&data->ornt_enable) == 1 || atomic_read(&data->en_double_tap) == 1)
#elif defined(CONFIG_DOUBLE_TAP)
	if(atomic_read(&data->en_double_tap) == 1)
#else
	if(atomic_read(&data->ornt_enable) == 1)
#endif
	{
		irq_enabled_in_suspend = true;
		enable_irq_wake(data->IRQ);		
	} else irq_enabled_in_suspend = false;
	suspend_flag = 1;
#endif

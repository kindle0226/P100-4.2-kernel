...
static struct wake_lock omap_serial_wake_lock;
...
static inline irqreturn_t serial_omap_irq(int irq, void *dev_id)
{
	...
	if (int_id == UART_IIR_RDI || int_id == UART_OMAP_IIR_RX_TIMEOUT ||
		...
		wake_lock_timeout(&omap_serial_wake_lock, 1*HZ);
		if (!up->use_dma) {
			if (lsr & UART_LSR_DR)
...
static int __init serial_omap_init(void)
{
	...
	wake_lock_init(&omap_serial_wake_lock, WAKE_LOCK_SUSPEND, "omap-serial");

...
static DEVICE_ATTR(enable, S_IWUSR | S_IRUGO |S_IRGRP |S_IXGRP |S_IROTH |S_IWOTH, get_enable, set_enable);
static DEVICE_ATTR(sampling_freq, S_IWUSR | S_IRUGO |S_IRGRP |S_IXGRP |S_IROTH |S_IWOTH, get_sampling_freq,
							set_sampling_freq);
static DEVICE_ATTR(polling_rate,S_IWUSR | S_IRUGO |S_IRGRP |S_IXGRP |S_IROTH |S_IWOTH, get_polling_rate,
							set_polling_rate);
static DEVICE_ATTR(reset_steps, S_IWUSR, NULL, reset_steps);
static DEVICE_ATTR(max_delivery_rate, S_IWUSR | S_IRUGO |S_IRGRP |S_IXGRP |S_IROTH |S_IWOTH, get_max_delivery_rate,
...
static DEVICE_ATTR(scale, S_IWUSR | S_IRUGO |S_IRGRP |S_IXGRP |S_IROTH |S_IWOTH, get_cur_scale, set_cur_scale);  
...
int lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq, u16 bustype)
{
	...
	u8 wai =0x00;
	struct lsm6ds3_sensor_data *sdata;
		dev_err(cdata->dev, "lsm6ds3_common_probe 1111.\n");
	mutex_init(&cdata->bank_registers_lock);
		dev_err(cdata->dev, "lsm6ds3_common_probe 2222.\n");
...
dev_err(cdata->dev, "irq = %d .\n",irq);
	if (irq > 0) {
#ifdef CONFIG_OF
		err = lsm6ds3_parse_dt(cdata);
		if (err < 0){
			dev_err(cdata->dev, "lsm6ds3_parse_dt error  .\n");
			return err;
			}	
#else /* CONFIG_OF */
	...
	if (gpio_request(irq, "lsm6ds3") < 0)  {
		dev_err(cdata->dev, "GPIO request failed\n");
	}
	gpio_direction_input(irq);
	cdata->irq = gpio_to_irq(irq);
	}
